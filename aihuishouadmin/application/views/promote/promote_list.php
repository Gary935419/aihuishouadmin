		...
		<div class="layui-col-md12">
			<div class="layui-card">
				<div class="layui-card-body ">
					<form class="layui-form layui-col-space5" method="get" action="<?= RUN, '/promote/promote_list' ?>">
						<div class="layui-inline layui-show-xs-block">
							<input type="text" name="user_name" id="user_name" value="<?php echo $user_name1 ?>"
								   placeholder="优惠卷名" autocomplete="off" class="layui-input">
						</div>
						<div class="layui-inline layui-show-xs-block">
				...
				<button class="layui-btn layui-card-header" style="float: right;margin-top: -40px;margin-right: 20px;"
						onclick="xadmin.open('添加','<?= RUN . '/promote/promote_add' ?>',900,600)"><i
								...
								<tr id="p<?= $once['pid'] ?>" sid="<?= $once['pid'] ?>">
									...
									<td class="td-manage">
										<button class="layui-btn layui-btn-normal"
												onclick="xadmin.open('编辑','<?= RUN . '/promote/promote_edit?id=' ?>'+<?= $once['pid'] ?>,900,700)">
											<i class="layui-icon">&#xe642;</i>编辑
										</button>
										<button class="layui-btn layui-btn-danger"
												onclick="promote_delete('<?= $once['pid'] ?>')"><i class="layui-icon">&#xe640;</i>删除
										</button>
									</td>
								</tr>
							<?php endforeach; ?>
		...
		layer.confirm('您是否确认删除？', {
				...
				function (index) {
					$.ajax({
						...
						data: {"id": id},
						dataType: "json",
						url: "<?= RUN . '/promote/promote_delete' ?>",
