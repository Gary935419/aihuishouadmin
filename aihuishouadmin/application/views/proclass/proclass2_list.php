		...
		<div class="layui-col-md12">
			<div class="layui-card">
				<div class="layui-card-body ">
					<form class="layui-form layui-col-space5" method="get" action="<?= RUN, '/proclass/proclass2_list' ?>">
						<div class="layui-inline layui-show-xs-block">
							<input type="text" name="user_name" id="user_name" value="<?php echo $user_name1 ?>"
								   placeholder="标签名" autocomplete="off" class="layui-input">
						</div>
						<div class="layui-inline layui-show-xs-block">
					...
					<table class="layui-table layui-form">
						...
						<?php if (isset($list) && !empty($list)) { ?>
							<?php foreach ($list as $num => $once): ?>
								<tr id="p<?= $once['ct_id'] ?>" sid="<?= $once['ct_id'] ?>">
									<td><?= $num + 1 ?></td>
									<td><?= $once['co_name'] ?></td>
									<td><?= $once['ct_name'] ?></td>
									<td><?= $once['ct_price'] ?></td>
									...
									<td class="td-manage">
										...
										<button class="layui-btn layui-btn-danger"
												onclick="lable_delete('<?= $once['ct_id'] ?>')"><i class="layui-icon">&#xe640;</i>删除
										</button>
									</td>
								</tr>
							<?php endforeach; ?>
		...
		layer.confirm('您是否确认删除？', {
				...
				function (index) {
					$.ajax({
						...
						data: {"id": id},
						dataType: "json",
						url: "<?= RUN . '/proclass/proclass2_delete' ?>",
